  ...
  normalize_options: function (options) {
    ...
    setup_fieldname_select: function (frm) {
      ...
      frappe.model.with_doctype(frm.doc.document_type, function () {
        ...
        var grid = frm.fields_dict.recipients.grid;

        var document_field_options = frappe.notification.normalize_options(
          [""].concat(["owner"]).concat(receiver_fields)
        );

        var employee_field_options = frappe.notification.normalize_options(
          [""].concat(employee_receiver_fields)
        );

        grid.docfields.forEach(function (df) {
          if (df.fieldname === "receiver_by_document_field") {
            df.options = document_field_options.join("\n");
          }

          if (df.fieldname === "receiver_by_employee_field") {
            df.options = employee_field_options.join("\n");
          }
        });
