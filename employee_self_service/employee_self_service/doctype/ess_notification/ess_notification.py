...
from frappe.utils import add_to_date, nowdate, validate_email_address,get_link_to_form
from frappe.utils.jinja import validate_template
from frappe.utils.safe_exec import get_safe_globals
from employee_self_service.employee_self_service.doctype.ess_notification.v12_compatible import *
...
class ESSNotification(Document):
    ...
    def validate_condition(self):
        ...
        if self.condition_expression:
            try:
                frappe.safe_eval(self.condition_expression, None, get_context(temp_doc.as_dict()))
            except Exception:
                frappe.throw(_("The Condition '{0}' is invalid").format(self.condition_expression))
    ...
    def get_documents_for_today(self):
        ...
        for d in doc_list:
            ...
            if self.condition_expression and not frappe.safe_eval(
                self.condition_expression, None, get_context(doc)
            ):
                continue
    ...
    def get_list_of_recipients(self, doc, context):
        ...
        for recipient in self.recipients:
            if recipient.condition_expression:
                if not frappe.safe_eval(recipient.condition_expression, None, context):
                    continue
...
def evaluate_alert(doc: Document, alert, event):
    ...
    try:
        ...
        if alert.condition_expression:
            if not frappe.safe_eval(alert.condition_expression, None, context):
                return
        ...
        if event == "Value Change" and not doc.is_new():
            ...
            field_value_before_save = (
                doc_before_save.get(alert.value_changed) if doc_before_save else None
            )

            fieldtype = doc.meta.get_field(alert.value_changed).fieldtype
            if cast(fieldtype, doc.get(alert.value_changed)) == cast(
        ...
        ).format(get_link_to_form("ESS Notification", alert.name))
