...
class NoTeamLeaderError(Document):
	...
	def _populate_nearest_team_leader_info(self):
		"""Find the employee's last reports-to leader (internal or external)
		for today up to the error time and store distance + coords on this record."""
		try:
			emp_lat = float(self.latitude)
			emp_lon = float(self.longitude)
		except (ValueError, TypeError):
		...
		ntl_update = {
			"nearest_team_leader": manager_id,
			"nearest_team_leader_name": manager_name,
		}
		...
		if row:
			...
			if m_lat is not None:
				distance = self._haversine(emp_lat, emp_lon, m_lat, m_lon)
				ntl_update.update({
					"nearest_team_leader_checkin": row[0].checkin_time,
					"nearest_team_leader_latitude": str(m_lat),
					"nearest_team_leader_longitude": str(m_lon),
					"nearest_team_leader_distance": str(round(distance, 2)) + " m",
				})
...
def get_all_leaders_with_distance(docname):
	...
	# Sort by checkin time descending (latest first)
	results.sort(key=lambda x: x["checkin_time"], reverse=True)
	return results
