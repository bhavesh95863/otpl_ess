...
def update_worker_fields_from_manager(doc, method=None):
	...
	- Update their fields from their reporting manager
	...
	try:
		...
		frappe.log_error(
			...
			title=f"Error in update_worker_fields_from_manager for Employee {doc.name}"
		)
...
def _update_single_worker_from_manager(doc):
	...
	try:
		...
		if not manager_data:
			frappe.log_error(
				f"Could not fetch manager data for Employee: {reporting_manager}",
				"Worker Field Update Error"
			)
			return
		
		# Check if manager has external sales order
		if manager_data.get("external_sales_order") == 1:
			# Manager has external sales order
			doc.external_sales_order = 1
			...
			# Manager has internal sales order
		...
		frappe.log_error(
			message=frappe.get_traceback(),
			title=f"Error updating worker fields for Employee {doc.name}"
		)
	...
	try:
		...
		for worker in workers:
			try:
				...
				frappe.log_error(
					message=frappe.get_traceback(),
					title=f"Error updating worker {worker.name} under team leader {team_leader_doc.name}"
				)
		...
		frappe.log_error(
			message=frappe.get_traceback(),
			title=f"Error updating workers under team leader {team_leader_doc.name}"
		)
	...
	try:
		...
		for worker in workers:
			try:
				...
				# Set external fields from Employee Pull
				worker_doc.external_sales_order = 1
				...
				frappe.log_error(
					message=frappe.get_traceback(),
					title=f"Error updating worker {worker.name} from Employee Pull {doc.name}"
				)
		...
		frappe.log_error(
			message=frappe.get_traceback(),
			title=f"Error updating workers from Employee Pull {doc.name}"
		)
