...
def auto_checkout_workers():
	"""
	Auto checkout for Workers (location != Site) who checked in but didn't check out
	Mark them as absent and create auto checkout at day end
	Called by scheduled job at end of day
	"""
	today = getdate()
	
	# Get all active Workers with location != Site who checked in today
	workers = frappe.db.sql("""
		SELECT DISTINCT ec.employee, e.employee_name, e.location
		FROM `tabEmployee Checkin` ec
		INNER JOIN `tabEmployee` e ON ec.employee = e.name
		WHERE e.staff_type = 'Worker'
		AND e.location != 'Site'
		AND e.status = 'Active'
		AND DATE(ec.time) = %s
		AND ec.log_type = 'IN'
	...
	for worker in workers:
		try:
			# Get ESS Location settings
			location_settings = None
			if worker.location and frappe.db.exists("ESS Location", worker.location):
				location_settings = frappe.get_doc("ESS Location", worker.location)
			
			# Default checkout time is shift end or 6 PM
			checkout_time = time(18, 0, 0)  # 6:00 PM
			if location_settings and location_settings.shift_end_time:
				checkout_time = location_settings.shift_end_time
			
			# Create auto checkout at shift end time
			checkout_datetime = datetime.combine(today, checkout_time)
			
			# Create Employee Checkin for OUT
			checkin_doc = frappe.get_doc({
				"doctype": "Employee Checkin",
				"employee": worker.employee,
				"time": checkout_datetime,
				"log_type": "OUT",
				"device_id": "Auto Checkout",
			...
			frappe.log_error(
				title="Auto Checkout Error: {0}".format(worker.employee),
				message=frappe.get_traceback()
			)
	
	frappe.logger().info("Auto checkout completed for {0} workers".format(processed_count))
